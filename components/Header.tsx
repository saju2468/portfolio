...
/**
 * MobileMenu sub-component for better separation of concerns and performance
 */
const MobileMenu = ({
  onClose,
  mounted,
  ...
  return (
    ...
    >
      ...
      <div 
        className="absolute inset-0 pointer-events-none flex justify-center items-center opacity-60 dark:opacity-20" 
        aria-hidden="true"
      >
      ...
      <div className="w-full h-px bg-black/20 dark:bg-white/20 mt-6 mb-8 md:mb-12 relative z-10" />
    ...
    <>
      <header className="fixed top-0 left-0 w-full p-6 md:p-12 z-[1000] flex justify-between items-baseline mix-blend-difference text-white backdrop-blur-md">
        <div className="flex flex-col md:flex-row md:items-baseline gap-2 md:gap-12">
          <Link href="/" aria-label="Sajin Portfolio Home">
        ...
        <nav aria-label="Primary Navigation" className="flex items-center gap-6 md:gap-10">
          <ul className="hidden lg:flex items-center gap-16 text-[10px] uppercase tracking-[0.4em] font-medium opacity-60">
            {NAV_LINKS.map((link) => (
              <li key={link.name} className="hover:opacity-100 cursor-pointer transition-all hover:text-[#ff4d00]">
                <Link
                  href={link.href}
          ...
          <div className="flex items-center gap-6 md:gap-10">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-1 w-4 h-4 translate-y-[2px]">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className="w-full h-full border border-white transition-all" />
                ))}
              </div>
            </button>
          </div>
