...
export function Work() {
  ...
  // Smooth cursor tracking for the floating image
  ...
  return (
    ...
    >
        ...
        >
          {projects.map((project, index) => (
            <Image
              ...
              className={`object-cover transition-all duration-700 ease-in-out ${hoveredIndex === index ? 'opacity-100 scale-100' : 'opacity-0 scale-110'}`}
            />
          ))}
      ...
      <div className="flex flex-col gap-20">
        ...
        {/* Premium Typographic List */}
        <div className="w-full flex flex-col mt-4">
          <div className="border-t border-grid-line w-full" />
          ...
          {projects.map((project, index) => {
            ...
            return (
              <FadeIn key={project.id} delay={0.1 * (index + 1)}>
                <Link href={project.link} target="_blank" rel="noopener noreferrer" className="block w-full group/link">
                  <div
                  ...
                  >
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between relative z-10 w-full">
                      ...
                      <div className="flex items-center gap-6 lg:gap-16 relative">
                        ...
                        <h3 
                          className={`text-5xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl font-black tracking-tighter uppercase leading-[0.8] transition-all duration-700 group-hover/row:translate-x-4 ${!isHovered && 'dark:opacity-40 opacity-50'}`} 
                          style={{ 
                      ...
                      <div className="flex flex-col gap-3 lg:items-end w-full lg:w-auto mt-6 lg:mt-0 transition-all duration-500 lg:opacity-0 lg:translate-x-8 group-hover/row:opacity-100 group-hover/row:translate-x-0">
                        ...
                        <div className="flex flex-wrap gap-2 lg:justify-end">
                           {project.tags.slice(0, 3).map((tag, tagIndex) => (
                             <span 
                               key={tag} 
                               className="text-[10px] font-mono border border-grid-line text-foreground px-2 py-1 uppercase backdrop-blur-md bg-background transition-all duration-500 delay-[0ms] font-medium shadow-sm"
